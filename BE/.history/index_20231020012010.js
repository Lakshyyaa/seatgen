...
function addcollumn(sheetname, collumnname) {
    ...
    const sheetName = workbook.SheetNames[0]
    const sheet = workbook.Sheets[sheetName]
    // const columnLetter = String.fromCharCode(65 + Object.keys(sheet).length / (sheet['1'].v ? 2 : 1)); // Determine the column letter based on existing data
    const newCell = `${collumnname}1`;
    sheet[newCell] = { t: 's', v: collumnname };
